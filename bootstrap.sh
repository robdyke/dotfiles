...
if [ $(uname) == 'Darwin' ]; then
    ...
    sudo apt-get -y install tmux vim git ssh language-pack-en figlet httpie ncdu tree wget
    ...
    # figlet is for server-splash
    sudo apt-get -y install tmux vim git ssh figlet httpie ncdu tree wget
    sudo ln -sf /usr/share/zoneinfo/Europe/London /etc/localtime
else
    echo "Unsupported OS."
    exit 2
