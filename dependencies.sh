...
if [ -x /usr/bin/apt-get ]; then
	...
	yes | sudo apt-get install picocom vim git tmux build-essential keychain mcabber htop || exit 2
elif [ -x /usr/local/bin/brew ]; then
	# mac os x (vim comes out-of-the-box, yay!)
	brew update || exit 2
	brew install git tmux keychain picocom mcabber htop-osx nodejs
