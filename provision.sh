...
if [ $(uname) == 'Darwin' ]; then
    ...
    packages=(tmux vim git httpie ncdu tree bash wget task htop gnupg2 bash-completion keychain iproute2mac)
    ...
    sudo apt-get -y install tmux vim git ssh language-pack-en figlet httpie ncdu tree wget htop gnupg2 curl keychain
    ...
    # figlet is for server-splash
    sudo apt-get -y install tmux vim git ssh figlet httpie ncdu tree wget htop gnupg2 keychain
    sudo ln -sf /usr/share/zoneinfo/Europe/London /etc/localtime
else
    echo "Unsupported OS."
    exit 2
